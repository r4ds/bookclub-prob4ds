...
- It is a *measure* of the size of a set.

`r knitr::include_graphics("images/ProbLaw.png")`
...
`r knitr::include_graphics("images/union.jpg")`
...
$$
The union bound is frequently used for analzying problems where the intersection is difficult to evaluate (e.g. multiple hypothesis testing)


## Conditional Probability

In data science ee are often interested in the relationship between two or more events. 

Question: If $A$ has happened, what is the probability that $B$ also happens? 
...
`r knitr::include_graphics("images/cond_prob_example.jpg")`
...
`r knitr::include_graphics("images/two_dice.jpg")`
...
`r knitr::include_graphics("images/Monty_open_door.png")`

To make this concrete consider the "Monty Hall Problem* (very similar to the three prisoners problem in the book).

- Three doors, behind one is a prize, behind the other door there are goats (or some other worthless prize)
- Contestant picks a door. He has a 1/3 chance of picking the door with the prize. 
- Monty then opens one of the other doors, revealing a goat. Do you switch? 
...
$$
\mathbb{P}[D_1|M_3] = \frac{\mathbb{P}[M_3 \mid D_1]\mathbb{P}[D_1]}{\mathbb{P}[M_3 \mid D_1]\mathbb{P}[D_1] + \mathbb{P}[M_3 \mid D_2]\mathbb{P}[D_2] + \mathbb{P}[M_3 \mid D_3]\mathbb{P}[D_3]} = \frac{1/2*1/3}{1/2*1/3+1/3+0}= 1/3
$$



Note that $\mathbb{P}[D_1|M_3] = \mathbb{P}[D_1]$ and is independat of the door opening. So you should switch!  Surprisingly, in a *Mythbusters* episode 20 out of 20 volunteers decided not to switch!
...
monty_1 <- dplyr::tibble(
   ...
   door_monte_chooses = dplyr::case_when(
...
Now condition on  the observation "door #2 chosen":

```{r}
monty_1 |>
  dplyr::filter(door_monte_chooses == 2) |>
...
- Monty only asks you to switch when he knows you have the prize... in that case never switch.

- Monty doesn't know where the prize is! In that case it turns out the probability you have the prize is 0.50 and it doesn't matter if you swithch.  Let's sim!

```{r}
num_samples = 10000
monty_1 <- dplyr::tibble(
   door_with_prize = sample(c(1,2,3), num_samples, replace= TRUE),
   door_monte_chooses = sample(c(2,3),num_samples, replace = TRUE) 
)

# We observe open door is #2 and no prize there: 
monty_1 |>
      dplyr::filter(door_monte_chooses == 2 & door_with_prize != 2) |>
      dplyr::summarize(mean(door_with_prize == 1))
