...
The text reviews this quite well, with the result being that we can *diagonalize* the covariance matrix:

$$
\mathbf{\Sigma} = \mathbf{U\Lambda U^T}
$$

where $\Lambda$ is diagonal, and the columns are $U$ are orthonormal ($U^TU=I$). If the columns of $U$ are $u_i$ then these can serve as a basis:

$$
\begin{align}
&\mathbf{x} = \sum_{j=1}^N\alpha_j \mathbf{u}_j \\
\\
&\text{whith basis coefficients:}\\
\\
&\alpha_j = \mathbf{u}_j^T\mathbf{x}
\end{align}
$$

This allows us to understand the mutivarient gaussian: the basis vectors are the orientation of the ellipsoids and the eigenvalues are the width of the distribution along those basis vector directions.
